...
export default function LeadsDashboard({ sectionRequest = "profile" }: { sectionRequest?: string }) {
  ...
  const fetchPerfis = async () => {
    try {
      const res = await fetch("/api/ig/leads");
      const data = await res.json();
      setPerfis(data);
    } catch {}
  };
  ...
  const deleteSearch = async (pai: string) => {
    try {
      await fetch(`/api/ig/leads/buscas/${pai}`, { method: "DELETE" });
      fetchPerfis();
    } catch {}
  };
  ...
  const sendBulkDMs = async () => {
    ...
    try {
      const res = await fetch("/api/ig/leads/send-bulk-dms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
      ...
      if (!res.ok) {
        ...
        try {
          await fetch("/api/ig/leads/marcar-dm", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          ...
          >
            ...
            >
              ...
              <div className="max-h-[60vh] overflow-y-auto custom-scrollbar divide-y divide-slate-800/40">
                {(Array.from(new Set(perfis.map(p => p.perfil_pai).filter(Boolean))) as string[]).map((keyword, i) => {
                  const followerCount = perfis.filter(p => p.perfil_pai === keyword).length;
                  const firstPerfil = perfis.find(p => p.perfil_pai === keyword);
                  const isConfirmingDelete = deletingId === keyword;
                  return (
                    <motion.div
                      key={keyword || i}
                    ...
                    >
                      <div className="flex-1 flex items-center gap-4 text-left min-w-0">
                        <div className="p-2 bg-slate-800 rounded-lg shrink-0 group-hover:bg-purple-500/20 transition-colors">
                          <Search className="w-4 h-4 text-slate-500 group-hover:text-purple-400 transition-colors" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-white text-sm font-semibold truncate">
                            Busca: <span className="text-purple-400">"{keyword}"</span>
                          </p>
                          <div className="flex items-center gap-3 mt-0.5">
                            <span className="flex items-center gap-1 text-[10px] text-slate-500">
                              <Users className="w-3 h-3" />{followerCount} leads
                            </span>
                            <span className="flex items-center gap-1 text-[10px] text-slate-600">
                              <Clock className="w-3 h-3" />{firstPerfil ? new Date(firstPerfil.criado_em).toLocaleString("pt-BR") : ''}
                            </span>
                          </div>
                        </div>
                      ...
                      <div className="shrink-0 flex items-center gap-1 z-10" onClick={e => e.stopPropagation()}>
                        {isConfirmingDelete ? (
                          <div className="flex items-center gap-1.5 bg-red-950/40 border border-red-500/30 px-2 py-1 rounded-lg text-[10px] font-bold text-red-400">
                            ...
                            <button onClick={e => { e.stopPropagation(); deleteSearch(keyword!); }} className="px-1.5 py-0.5 bg-red-500 text-slate-950 rounded hover:bg-red-400 transition-colors cursor-pointer">
                              Sim
                            </button>
                          ...
                          <button onClick={e => { e.stopPropagation(); setDeletingId(keyword!); }} className="p-2 text-slate-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all cursor-pointer" title="Excluir busca e liberar espaço">
      ...
      <header className="flex justify-between items-start p-8 pb-0 shrink-0">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-3xl font-bold text-white tracking-tight">Leads Qualificados</h2>
          </div>
          <p className="text-slate-400 text-sm mt-1">Busca Profunda no Google (Dorks) + Extração Stealth de perfis do Instagram.</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          {Array.from(new Set(perfis.map(p => p.perfil_pai).filter(Boolean))).length > 0 && (
            <button
            ...
            >
              ...
              <span className="bg-purple-500/20 text-purple-400 text-[10px] font-black px-2 py-0.5 rounded-full ml-1">
                {Array.from(new Set(perfis.map(p => p.perfil_pai).filter(Boolean))).length}
              </span>
            </button>
          )}
        ...
        <div className="glass rounded-2xl p-6 flex flex-col gap-4 shadow-2xl relative overflow-hidden border border-slate-800">
          ...
          <AnimatePresence>
            <div className="flex items-start gap-3 mt-2 p-3 text-pink-400 text-sm font-bold border-2 border-pink-500/50 rounded-xl bg-pink-500/10 animate-[pulse_2s_ease-in-out_infinite] shadow-[0_0_15px_rgba(236,72,153,0.2)]">
              ...
              <p>
                ATENÇÃO: Ao iniciar a busca, o navegador será aberto. Pode ser exigida a resolução manual de um CAPTCHA (teste de imagem). Fique atento à janela do robô!
              </p>
            </div>
                  ...
                  return (
                    <>
                      <div className="flex items-center justify-between px-6 py-2 bg-slate-950/50">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-black uppercase tracking-widest text-pink-400/70">
                            {leadsToShow.length} Leads Encontrados
                          </span>
                        </div>
                      ...
                      {leadsToShow.slice(0, visibleFollowersMap['all'] || 25).map(p => {
                        ...
                        return (
                          <div key={p.id} className={`grid grid-cols-[3fr_3fr_2fr_3fr_1.5fr] items-center px-6 py-4 hover:bg-slate-800/30 transition-colors ${bgClass}`}>
                            <div className="flex items-center gap-2">
                              <div className="flex items-center gap-2 mr-1">
                                <input 
                                  ...
                                  className="w-4 h-4 rounded border-slate-700 text-pink-500 focus:ring-pink-500/20 bg-slate-900 cursor-pointer"
                                />
                              </div>
                            ...
                            <div className="flex flex-col items-center justify-center text-center">
                              <p className={`text-[10px] text-slate-500 line-clamp-2 leading-relaxed`} title={p.bio}>
                                {p.bio || (<span className="italic">Sem biografia</span>)}
                              </p>
                            </div>
                            ...
                            <div className="flex items-center justify-center gap-2 flex-wrap">
                              ...
                              {p.link_bio ? (
                                ...
                                <span className="text-[10px] text-slate-600 font-mono font-bold">-</span>
                              )}
                              {/* Email */}
                              {p.email_extraido && (
                              ...
                              {p.telefone_extraido && (
                                (() => {
                                  ...
                                  if (isCelular) {
                                     return (
                                      <a href={getWhatsAppUrl(p.telefone_extraido)} target="_blank" rel="noreferrer" onClick={() => markDmEnviado(p.username)} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-md text-[10px] font-bold uppercase hover:bg-emerald-500 hover:text-slate-950 transition-all">
                      ...
                      {leadsToShow.length > (visibleFollowersMap['all'] || 25) && (
                        <div className="px-6 py-3 bg-slate-950/30 flex justify-center border-t border-slate-800/50">
                          <button 
                            onClick={() => setVisibleFollowersMap(prev => ({...prev, 'all': (prev['all'] || 25) + 25}))}
                          ...
                          >
                            ...
                            Carregar mais leads (+25 de {leadsToShow.length - (visibleFollowersMap['all'] || 25)} restantes)
                          </button>
                        </div>
                      )}
                    </>
